...
import com.beyondxscratch.mandaloreexpress.domain.search.spacetrain.fare.firstClassFareOption
import com.beyondxscratch.mandaloreexpress.domain.search.spacetrain.fare.randomFareOption
import com.beyondxscratch.mandaloreexpress.domain.search.spacetrain.fare.secondClassFareOption
...
fun randomSpaceTrain(): SpaceTrain = spaceTrain()
    .copy(
        ...
        fareOptions = setOf(randomFareOption(), randomFareOption()),
        ...
        .mapIndexed { journeyIndex, journey ->
            (1..2).map {
                SpaceTrain(
                    ...
                    schedule = schedule(),
                    fareOptions = setOf(firstClassFareOption(), secondClassFareOption())
                )
            }
        }
        .flatten()
...
private fun List<SpaceTrain>.generateCompatibleSpaceTrainsFrom(criteria: Criteria): List<SpaceTrain> {
    ...
    return outbounds.mapIndexed { index, spaceTrain -> spaceTrain.copy(compatibleSpaceTrains = setOf(inbounds[index].number)) } + inbounds.mapIndexed { index, spaceTrain ->
        spaceTrain.copy(
            compatibleSpaceTrains = setOf(outbounds[index].number)
        )
    }
}
