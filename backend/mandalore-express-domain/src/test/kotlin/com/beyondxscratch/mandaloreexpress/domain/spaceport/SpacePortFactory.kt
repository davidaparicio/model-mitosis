...
fun randomSpacePort(): SpacePort =
    SpacePort(id = randomUUID().toString(), name = randomUUID().toString(), location = values().random())

fun randomSpacePort(planet: Planet): SpacePort =
    spacePorts.getAllSpacePorts().filter { it.location == planet }.drop(1).random()
